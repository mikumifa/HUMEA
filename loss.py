...
class icl_loss(nn.Module):
    ...
    def forward(self, emb, train_links, emb2=None, norm=True):
        ...
        logits_aa = (
            torch.matmul(hidden1, torch.transpose(hidden1_large, 0, 1)) / temperature
        )
        logits_aa = logits_aa - masks * LARGE_NUM
        logits_bb = (
            torch.matmul(hidden2, torch.transpose(hidden2_large, 0, 1)) / temperature
        )
...
class ial_loss(nn.Module):
    ...
    def forward(self, src_emb, tar_emb, train_links, norm=True):
        ...
        loss_a = F.kl_div(
            F.log_softmax(p_ab, dim=1),
            F.softmax(q_ab.detach(), dim=1),
            reduction="none",
        )
        loss_b = F.kl_div(
            F.log_softmax(p_ba, dim=1),
            F.softmax(q_ba.detach(), dim=1),
            reduction="none",
        )
